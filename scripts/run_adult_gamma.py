...
for seed in seeds:
    out_file = f'logs/adult/adult_{seed}.csv'
    ...
    for gamma in [0, 0.02, 0.1, 0.2, 0.9]:
        ...
        cmd = f'python train_enc_categorical.py --dataset adult --seed {seed} --batch_size 128 --weight_decay {weight_decay} --n_epochs 100 --adv_epochs 100 --gamma {gamma} --p_val {p_val} --p_test {p_test} --out_file {out_file} --verbose --encode'
        if args.with_test:
            cmd += ' --with_test'
        print(cmd)
        os.system(cmd)
