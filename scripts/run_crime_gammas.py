...
for seed in seeds:
    out_file = f'logs/crime/crime_{args.scalarization}_{seed}.csv'
    ...
    for gamma in [0, 0.02, 0.1, 0.2, 0.9]:
        ...
        cmd = f'python crime_flow_multi.py --prior gmm --batch_size {batch_size} --n_epochs {n_epochs} --adv_epochs {adv_epochs} --gamma {gamma} --seed {seed} --train_dec --kl_start {kl_start} --kl_end {kl_end} --log_epochs {log_epochs} --gmm_comps1 {gmm_comps1} --gmm_comps2 {gmm_comps2} --lr {lr} --weight_decay {weight_decay} --n_blocks {n_blocks} --p_val {p_val} --p_test {p_test} --out_file {out_file} --scalarization {args.scalarization}'
        if args.with_test:
            cmd += ' --with_test'
        print(cmd)
        os.system(cmd)
